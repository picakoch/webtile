...
(function (global, factory) {
  ...
  function removeClasses(element, clsRegex) {
    ...
    for (const node of toNodes(element)) {
      node.classList.remove(
        ...toArray(node.classList).filter((cls) => cls.match(clsRegex)),
      );
    }
  }
  ...
  function closest(element, selector) {
    ...
    return (_a = toNode(element)) == null
      ? void 0
      : _a.closest(startsWith(selector, ">") ? selector.slice(1) : selector);
  }
  ...
  function scrollIntoView(element, { offset: offsetBy = 0 } = {}) {
    ...
    return parents2.reduce(
      (fn, scrollElement, i) => {
        ...
        return () =>
          scrollTo(scrollElement, top - scrollTop, element, maxScroll).then(fn);
      },
      () => Promise.resolve(),
    )();
    function scrollTo(element2, top, targetEl, maxScroll) {
      return new Promise((resolve) => {
        ...
        const targetTop =
          offset(targetEl).top + (isScrollingElement ? 0 : scroll);
        let prev = 0;
        let frames = 15;
        (function step() {
          ...
          if (parents2[0] === element2 && scroll + top < maxScroll) {
            diff =
              offset(targetEl).top +
              (isScrollingElement ? 0 : element2.scrollTop) -
              targetTop;
  ...
  function scrollParents(element, scrollable = false, props = []) {
    ...
    return [scrollEl]
      .concat(
        ancestors.filter(
          (parent2) =>
            css(parent2, "overflow")
              .split(" ")
              .some((prop) => includes(["auto", "scroll", ...props], prop)) &&
            (!scrollable ||
              parent2.scrollHeight > offsetViewport(parent2).height),
        ),
      )
      .reverse();
  }
  ...
  function getCoveringElement(target) {
    ...
    for (const topPosition of [0, top]) {
      const coverEl = target.ownerDocument
        .elementsFromPoint(left + width / 2, topPosition)
        .find(
          (el) =>
            !el.contains(target) && // If e.g. Offcanvas is not yet closed
            ...
            ((hasPosition(el, "fixed") &&
              zIndex(
                parents(target)
                  ...
                  .find(
                    (parent2) =>
                      !parent2.contains(el) && !hasPosition(parent2, "static"),
                  ),
              ) < zIndex(el)) ||
  ...
  function viewport(options) {
    return observe(
      (target, handler) => observeViewportResize(handler),
      options,
      "resize",
    );
  }
  ...
  function toScrollTargets(elements) {
    return toNodes(elements).map((node) => {
      ...
      return parent2 === ownerDocument.scrollingElement
        ? ownerDocument
        : parent2;
    });
  }
  ...
  function getRows(elements) {
    ...
    for (const el of elements) {
      ...
      for (let i = sorted.length - 1; i >= 0; i--) {
        ...
        if (
          offset.top >= offsetCurrent.bottom - 1 &&
          offset.top !== offsetCurrent.top
        ) {
          sorted.push([el]);
          break;
        }
        if (
          offset.bottom - 1 > offsetCurrent.top ||
          offset.top === offsetCurrent.top
        ) {
  ...
  function getTransitionProps(target, nodes, currentProps) {
    ...
    const propsFrom = propsTo.map((props, i) => {
      const from =
        ...
        (currentProps[i] || getProps$1(nodes[i]));
      if (!from) {
        return false;
      }
      if (!props) {
    ...
    const off = on(el, "touchstart", (e) => {
      ...
      const offMove = on(
        ...
        (e2) => {
          ...
          prev = pos;
          if (
            !scrollParents(e2.target).some((scrollParent) => {
  ...
  function registerEvent(instance, event, key) {
    ...
    if (
      !el ||
      (isArray(el) && !el.length) ||
      (filter && !filter.call(instance))
    ) {
      return;
    }
      ...
      transformFn2 = (stop) => {
        ...
        return getUnit([stop])
          ? toPx(stop, "width", el, true) /
              el[
                `offset${((_a = stop.endsWith) == null ? void 0 : _a.call(stop, "vh")) ? "Height" : "Width"}`
              ]
          : toFloat(stop);
      };
    }
  ...
  function backgroundCoverFn(prop, el, stops, props2) {
    ...
    for (const prop2 of bgProps) {
      ...
      positions[prop2] =
        `max(${getBackgroundPos(el, prop2)},-${overflow}px) + ${positions[prop2]}`;
    }
  ...
  function setBackgroundPosFn(bgProps, positions, props2) {
    return function (css2, percent) {
      for (const prop of bgProps) {
        ...
        css2[`background-position-${prop.substr(-1)}`] =
  ...
  var parallax = {
    ...
    computed: {
      target: ({ target }, $el) =>
        getOffsetElement((target && query(target, $el)) || $el),
      start({ start }) {
  ...
  var SliderParallax = {
    ...
    update: {
      write() {
        ...
        const percent = ease(
          scrolledOver(target, start, end),
          this.parallaxEasing,
        );
        const [prevIndex, slidePercent] = this.getIndexAt(percent);
        const nextIndex = this.getValidIndex(
          prevIndex + Math.ceil(slidePercent),
        );
        const prev = this.slides[prevIndex];
        const next = this.slides[nextIndex];
        const { triggerShow, triggerShown, triggerHide, triggerHidden } =
          useTriggers(this);
        if (~this.prevIndex) {
          for (const i of /* @__PURE__ */ new Set([
            this.index,
            this.prevIndex,
          ])) {
            if (!includes([nextIndex, prevIndex], i)) {
        ...
        const changed =
          this.prevIndex !== prevIndex || this.index !== nextIndex;
        this.dir = 1;
        this.prevIndex = prevIndex;
        this.index = nextIndex;
        if (prev !== next) {
  ...
  function getWidth(list, index) {
    return sumBy(
      children(list).slice(0, index),
      (el) => dimensions$1(el).width,
    );
  }
        ...
        for (let i = 0; i < this.length; i++) {
          ...
          if (this.center) {
            if (
              ...
              left +
                slideWidth +
                dimensions$1(this.slides[getIndex(+i + 1, this.slides)]).width /
                  2 >
                width / 2
            ) {
              sets.push(+i);
    ...
    events: {
      beforeitemshow(e) {
        ...
        this.duration =
          ...
          (dimensions$1(this.slides[index]).width / avgWidth);
        this.reorder();
      },
      itemshow() {
        if (~this.prevIndex) {
    ...
    methods: {
      ...
      getIndexAt(percent) {
        ...
        const scrollDist = this.center
          ? getWidth(this.list) -
            (dimensions$1(this.slides[0]).width / 2 +
              dimensions$1(last(this.slides)).width / 2)
        ...
        do {
          ...
          const slideDist = this.center
            ? slideWidth / 2 + dimensions$1(this.slides[index + 1]).width / 2
            : slideWidth;
  ...
  function findTarget(items, point) {
    return items[
      findIndex(items, (item) => pointInRect(point, dimensions$1(item)))
    ];
  }
        ...
        on(this.tooltip, "toggled", (e, toggled) => {
          ...
          const handlers = [
            once(
              ...
              false,
              (e2) =>
                (e2.type === pointerDown$1 && !this.$el.contains(e2.target)) ||
  ...
  function getAlignment(el, target, [dir, align]) {
    ...
    align =
      props.find((prop) => elOffset[prop] === targetOffset[prop]) || "center";
    return [dir, align];
  }
  ...
  function getViewport$1(el, target) {
    return offsetViewport(
      overflowParents(target).find((parent2) => parent2.contains(el)),
    );
  }
  ...
  var Dropnav = {
      ...
      },
      {
        ...
        handler(e) {
          ...
          if (
            ...
            active2.$el === e.target &&
            this.isDropbarDrop(active2.$el) &&
            includes(active2.mode, "hover") &&
            active2.isDelayedHide &&
            !this.items.some(
    ...
    methods: {
      getActive() {
        var _a;
        return (
          includes(this.dropdowns, (_a = active) == null ? void 0 : _a.$el) &&
          active
        );
      },
        ...
        if (el) {
          ...
          if (diff > 0) {
            css(
              el,
              "transitionDelay",
              `${(diff / newHeight) * this.duration}ms`,
            );
          }
        }
        css(
      ...
      getDropbarOffset(offsetTop) {
        ...
        const { top, height: height2 } = offset(
          query(targetY || target || $el, $el),
        );
  ...
  var grid = {
    ...
    observe: scroll$1({
      filter: ({ parallax, parallaxJustify }) => parallax || parallaxJustify,
    }),
    update: [
      {
        write({ rows }) {
      ...
      },
      {
        read(data) {
          ...
          if (
            ...
            rows[0].some((el, i) =>
              rows.some(
                (row) => row[i] && row[i].offsetWidth !== el.offsetWidth,
              ),
            )
          ) {
          ...
          let translates;
          if (masonry) {
            [columns, translates] = applyMasonry(
              rows,
              gutter,
              masonry === "next",
            );
          ...
          const columnHeights = columns.map(
            (column) =>
              sumBy(column, "offsetHeight") + gutter * (column.length - 1),
          );
          ...
          if (parallax || parallaxJustify) {
            ...
            if (!parallaxJustify) {
              parallax = Math.max(
                ...columnHeights.map(
                  (hgt, i) => hgt + scrollColumns[i] - height,
                ),
              );
            }
  ...
  function getGutter(rows, cls) {
    ...
    return toFloat(
      node ? css(node, "marginTop") : css(rows[0][0], "paddingLeft"),
    );
  }
  ...
  var heightMatch = {
    ...
    observe: resize({
      target: ({ $el, elements }) =>
        elements.reduce(
          (elements2, el) => elements2.concat(el, ...el.children),
          [$el],
        ),
    }),
    events: {
    ...
    beforeConnect() {
      ...
      this.icon = isToggle
        ...
        : hasClass(this.$el, "uk-search-icon") &&
            closest(this.$el, ".uk-search-large")
          ? "search-large"
      ...
      for (const element of elements) {
        if (
          ...
          (element.closest('[class*="-leave"]') &&
            elements.some(
              (el) => element !== el && matches(el, '[class*="-enter"]'),
            ))
        ) {
          continue;
        }
  ...
  function install({ modal }) {
    ...
    modal.alert = function (message, options) {
      return openDialog(
        ({ i18n }) =>
          `<div class="uk-modal-body">${isString(message) ? message : html(message)}</div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-primary uk-modal-close" autofocus>${i18n.ok}</button> </div>`,
        options,
      );
    };
    modal.confirm = function (message, options) {
      return openDialog(
        ({ i18n }) =>
          `<form> <div class="uk-modal-body">${isString(message) ? message : html(message)}</div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-default uk-modal-close" type="button">${i18n.cancel}</button> <button class="uk-button uk-button-primary" autofocus>${i18n.ok}</button> </div> </form>`,
        options,
        () => Promise.reject(),
      );
    };
  ...
  var navbar = {
    ...
    methods: {
      ...
      getDropbarOffset(offsetTop) {
        ...
        return (
          top +
          (this.dropbarTransparentMode === "behind" ? 0 : height + offsetTop)
        );
      },
    },
  };
  ...
  var scrollspyNav = {
    ...
    computed: {
      links: (_, $el) =>
        $$('a[href*="#"]', $el).filter((el) => el.hash && isSameSiteAnchor(el)),
        ...
        read() {
          ...
          if (scrollTop === max) {
            ...
            for (let i = 0; i < targets.length; i++) {
              ...
              const offsetBy =
                this.offset + (fixedEl ? offset(fixedEl).height : 0);
  ...
  function parseProp(value, el, propOffset, padding) {
    ...
    if (isNumeric(value) || (isString(value) && value.match(/^-?\d/))) {
      ...
      return (
        ...
        (padding && (refElement == null ? void 0 : refElement.contains(el))
